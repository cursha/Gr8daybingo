...
const ResetPassword: React.FC = () => {
      ...
      <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: HERO_BG }}>
        <Card className="w-full max-w-md shadow-2xl border-0">
          <CardContent className="pt-8 text-center space-y-4">
            ...
            <p className="text-sm text-slate-500">Please request a new password reset link.</p>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await apiClient.post('/game/reset-password', { token, new_password: password }, { skipAuth: true });
      setDone(true);
          ...
          {done ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <p className="text-center text-sm text-slate-500">
                <Link to="/forgot-password" className="font-semibold text-indigo-600 hover:text-indigo-700 underline underline-offset-2">
