...
import { Inbox, Lightbulb, Check, XCircle, Trash2, FileSpreadsheet, Download, Upload, Printer, Sparkles, Heart, Plus, Save, X, Edit2, Search } from 'lucide-react';
...
const DeedsSection: React.FC<DeedsSectionProps> = ({ deedCategories }) => {
  ...
  const loadDeedsAndTargeting = async () => {
    try {
      const [deedsData, taRes] = await Promise.all([getAdminDeeds(), getAdminTargetingAttributes()]);
  ...
  const handleApprove = async (id: number) => {
    try {
      ...
      await Promise.all([loadPendingDeeds(pendingFilter), loadDeedsAndTargeting()]);
    } catch (err: any) {
...
<body>
  ...
  async function handleImportCsv(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const importedDeeds = rows.map((row) => {
      ...
      const result = await importDeeds(importedDeeds);
